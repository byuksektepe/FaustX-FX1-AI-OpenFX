...
class draw:
    ...
    def make_logo():
        ...
        img_logo = cv2.imread("parts/fx-logo-c.png", cv2.IMREAD_UNCHANGED)
    ...
    def make_logo_w():
        ...
        img_logo = cv2.imread("parts/fx-logo-w.png", cv2.IMREAD_UNCHANGED)
        img_logo = cv2.resize(img_logo, (0, 0), None, 0.5, 0.5)
    ...
    def draw_text_w_r(img, text, posx, posy, text_color, text_color_bg, rec, font, font_scale=0.4, font_thickness=1):
        ...
        cv2.putText(img, str(text), (posx-3, posy + text_h-3), font, font_scale, text_color, font_thickness, cv2.LINE_AA)
    ...
    def textBlurBackground(img, text, font, fontScale, textPos, textColor, textThickness=1,kneral=(63,63) , pad_x=3, pad_y=3):
    ...
    def GPI_VZ_ALT(img, color, fcx, fcy, vz):
        ...
        img = cv2.arrowedLine(img, (fcx+240, (fcy+2)+vz), (fcx+239, (fcy+2)+vz), color, 1, cv2.LINE_AA, 0, 8)
...
class text_details():
    ...
    def right_allign_text(self, img, text, color, center_x, center_y, 
        ...
        text = str(text)
        img = cv2.putText(img, (text), (center_x+(int(wf)//2)-(draw.make_text_size(img, (text), 0.4, font)+xpos), center_y-(int(hf)//2)+ypos), font, 0.4, color,1 ,cv2.LINE_AA)
...
class round_inducators:
    ...
    def draw_round_inducator_dynamic(self, img, xpos, ypos,
        ...
        axes = (radius, radius)
        angle = 270
        img = cv2.ellipse(img, (cf_cx+xpos, cf_cy+ypos), axes, angle, startAngle, endAngle, color, thickness, cv2.LINE_AA)
    ...
    def draw_round_inducator_main_static(self, img, xpos, ypos, 
        ...
        draw.draw_text_w_r(img, str(data_text), (cf_cx+xpos)-calc_text_pos_data, (cf_cy+ypos-8), color, incolor, 1, font)
        ...
        img = cv2.putText(img, str(style_text), ((cf_cx+xpos)-calc_text_pos_style, (cf_cy+ypos+20)), font, 0.3, color,1 ,cv2.LINE_AA)
        if(info_text):
            
            img = cv2.putText(img, str(info_text), ((cf_cx+xpos-37), (cf_cy+ypos)-35), font, 0.3, incolor,1 ,cv2.LINE_AA)

        # make rounds
        img = cv2.ellipse(img, (cf_cx+xpos, cf_cy+ypos), axes, angle, startAngle_R, endAngle_R, DarkedColor_RED, thickness, cv2.LINE_AA)
        img = cv2.ellipse(img, (cf_cx+xpos, cf_cy+ypos), axes, angle, startAngle_O, endAngle_O, DarkedColor_ORANGE, thickness, cv2.LINE_AA)
        img = cv2.ellipse(img, (cf_cx+xpos, cf_cy+ypos), axes, angle, startAngle_C, endAngle_C, DarkedColor_CYAN, thickness, cv2.LINE_AA)
        img = self.draw_round_inducator_dynamic(img, xpos, ypos, color, color_2, color_3, radius, startAngle, thickness, endAngle=Angle1)
...
class ahrs:
    ...
    def make_ahrs_compass(self, img, color, color_2, fcx, fcy, yaw, font):
        ...
        if(str(yaw)): # Ortadaki yaw datası
            ...
            img = cv2.putText(img, str(int(yaw)), (draw.make_text_center(img, str(int(yaw)), 0.5,1, font)+1, 35), font, 0.5, color,1 ,cv2.LINE_AA)
        ...
        for b in range (calculated_yaw-990, calculated_yaw+990, 90):
            ...
            img = cv2.putText(img, (self.to_yaw_string(fb_count)), (cf_cx-b-5-deger, 88), font, 0.5, color,1 ,cv2.LINE_AA)
        ...
        for a in range(calc_alt+80000, calc_alt-80000, -25):
            ...
            if(cf_cy+a < cf_cy-170 or cf_cy+a < cf_cy+170):
                ...
                if(int(value) % 10 == 0):
                    ...
                    if(value == alt):
                        img = cv2.line(img, (x_sabiti_start, cf_cy+a),  (x_sabiti_end, cf_cy+a), color_2, 1,cv2.LINE_AA)
                        img = cv2.putText(img, str(int(value)), (x_sabiti_end + 3, cf_cy+a+3), font, 0.3, color_2,1 ,cv2.LINE_AA)
                    elif(value >= 0):
                        img = cv2.line(img, (x_sabiti_start, cf_cy+a),  (x_sabiti_end, cf_cy+a), color, 1,cv2.LINE_AA)
                        img = cv2.putText(img, str(int(value)), (x_sabiti_end + 3, cf_cy+a+3), font, 0.3, color,1 ,cv2.LINE_AA)
                    else:
                        img = cv2.line(img, (x_sabiti_start, cf_cy+a),  (x_sabiti_end, cf_cy+a), color, 1,cv2.LINE_AA)
                        img = cv2.putText(img, str(int(value)), (x_sabiti_end + 3, cf_cy+a+3), font, 0.3, color,1 ,cv2.LINE_AA)
        ...
        for a in range(calc_speed+900, calc_speed-900, -25):
            ...
            if(cf_cy+a < cf_cy-170 or cf_cy+a < cf_cy+170):
                ...
                if(int(value) % 10 == 0):
                    ...
                    if(value == speed):
                        img = cv2.line(img, (x_sabiti_start, cf_cy+a),  (x_sabiti_end, cf_cy+a), color_2, 1,cv2.LINE_AA)
                        img = cv2.putText(img, str(int(value)), (x_sabiti_end - draw.make_text_size(img, str(int(value)), 0.3, font) -11, cf_cy+a+3), font, 0.3, color_2,1 ,cv2.LINE_AA)
                    elif(True):
                        img = cv2.line(img, (x_sabiti_start, cf_cy+a),  (x_sabiti_end, cf_cy+a), color, 1,cv2.LINE_AA)
                        img = cv2.putText(img, str(int(value)), (x_sabiti_end - draw.make_text_size(img, str(int(value)), 0.3, font) -11, cf_cy+a+3), font, 0.3, color,1 ,cv2.LINE_AA)
    ...
    def make_ahrs_roll_and_pitch_bars(self, img, color, 
        ...
        for a in range(720, -720, -30):
            ...
            if(int(value) % 10 == 5):
                ...
                txt_1_L_M = np.dot(rot, np.array([-47-draw.make_text_size(img, str(int(value)), 0.3, font), a+pitch-5])).astype(int)
                ...
                txt_1_L_P = np.dot(rot, np.array([-47-draw.make_text_size(img, str(int(value)), 0.3, font), a+pitch+12])).astype(int)
    ...
    def make_aoa_ssa_vector(self, img, color, color_2, color_3, aoa, ssa, climb, font):
        ...
        img = cv2.line(img, ((cf_cx+8)+ssa, (cf_cy)+aoa),  ((cf_cx+20)+ssa, (cf_cy)+aoa), color_3, 1,cv2.LINE_AA)
        img = cv2.line(img, ((cf_cx-8)+ssa, (cf_cy)+aoa),  ((cf_cx-20)+ssa, (cf_cy)+aoa), color_3, 1,cv2.LINE_AA)
        
        img = cv2.line(img, ((cf_cx)+ssa, (cf_cy)+aoa-8),  ((cf_cx)+ssa, (cf_cy)+aoa-20), color_3, 1,cv2.LINE_AA)
        #<-----
        #SHOW AOA DEG DATA AREA ----->
        draw.draw_text_w_r(img, ("AOA "+str(raw_aoa)), cf_cx-190-draw.make_text_size(img, "AOA "+str(raw_aoa), 0.4,font), cf_cy+205, color, color, 2, font)
        #<-----
